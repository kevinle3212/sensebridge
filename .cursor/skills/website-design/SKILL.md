...
| --- | --- | --- |
| **impeccable** | Execution + evaluation: production-grade design, the `craft`/`critique`/`audit`/`polish`/`typeset`/`layout`/… commands, AI-slop detection, the `website/` DESIGN.md/PRODUCT.md context and `.impeccable/` detectors | [.cursor/skills/impeccable/SKILL.md](../impeccable/SKILL.md) — already wired across every assistant surface |
