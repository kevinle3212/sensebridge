...
description:
    Use when adopting Swift 6.2 concurrency — single-threaded by default,
    `@concurrent` for explicit background offloading, isolated conformances for
    main-actor types. The mechanism behind the "main thread stays free" rule.
metadata:
...
If it is unclear whether work belongs on the main actor or offloaded, ask before
annotating. Isolation is a design decision, not a compiler-appeasement tactic.

# Swift 6.2 Approachable Concurrency

**Why this matters here:** `CLAUDE.md` makes "no perception or model inference on
the main thread; the UI must stay responsive to VoiceOver during processing" an
architecture invariant. This skill is how that invariant is enforced in code —
`@concurrent` is the deliberate offload, and VoiceOver responsiveness is the
reason to reach for it. Offloading here is an accessibility requirement, not a
