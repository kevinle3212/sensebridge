...
name: docker-clean
description:
    Prune Docker's build cache, unused images, stopped containers, and
    networks to reclaim disk space — safe by default, deep clean only with
    confirmation.
---
...
This repo's Docker footprint is production-relevant (Railway deploys read
`railway.toml`; local dev may read `docker-compose.yml` if one exists). Before
any image-removing step (tier 5, or `image prune -f` in tier 3 if it would
remove a tagged image), cross-check the image name/tag against references in
`railway.toml` and any `docker-compose*.yml` at the repo root — never prune an
image a deploy config still names, even if no container currently uses it.
