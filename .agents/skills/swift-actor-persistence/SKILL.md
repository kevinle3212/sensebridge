...
description:
    Use when building thread-safe local persistence in Swift — actor-guarded
    in-memory cache over file-backed storage, eliminating data races by design.
metadata:
...
Before persisting anything derived from the camera, microphone, or surroundings,
confirm it is meant to be stored at all. On this project the default answer is
no — ask rather than assume.

# Swift Actors for Thread-Safe Persistence

**Why this matters here:** persistence is a privacy surface before it is a
storage problem. `docs/PRIVACY.md` and the on-device invariant mean anything
written to disk must be justified, and anything derived from perception
(frames, recognised text, audio) is sensitive by default. Use this skill for
local state that has already been agreed; do not let "we need a cache" become
the reason perception data starts living on disk. Never persist secrets here —
those belong in the Keychain.
