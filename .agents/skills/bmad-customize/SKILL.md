...
- Every workflow a given agent runs → agent surface (e.g. `bmad-agent-architect.toml` with `persistent_facts`, `principles`).
- One workflow only → workflow surface (e.g. `bmad-architecture.toml` with `activation_steps_prepend`).
