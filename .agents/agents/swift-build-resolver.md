...
surgical changes. Use when `swift build` or `xcodebuild` fails.

**Green is not validated.** Clearing a build clears one of the gates in
`CLAUDE.md`, not the others. This agent must never imply a passing build means
the change is done: zero unlabeled elements, safety-framing review, and
model-license clearance are separate gates, and on-device latency, battery,
thermal, and blind-tester validation cannot be proven by any build at all.
Report what the compiler proved and nothing more.

**Never fix a build by weakening a doctrine.** Suppressing a data-race error
with `nonisolated`, moving perception work onto the main thread to dodge a
`Sendable` error, or adding a dependency to satisfy a resolve failure are not
build fixes — stop and report. `CLAUDE.md` and `AGENTS.md` outrank a green
build.
