...
and performance. Use for all Swift code changes.

Scope note: this agent owns *language-level* review. It does not replace the
project's doctrinal reviewers, and it never overrides them — route accordingly
and stack them:

- Any spoken, caption, or haptic string →
  [safety-framing-reviewer](safety-framing-reviewer.md). Highest-severity
  surface in the repo; this agent has no opinion there.
- Any new network call, permission, or data-at-rest →
  [security-reviewer](security-reviewer.md), which owns the on-device
  invariant.
- Any UI change → [accessibility-reviewer](accessibility-reviewer.md). Zero
  unlabeled elements is a hard gate, not a percentage.

**On-device caveat.** The generic checks below include ATS exceptions, SQL
injection, and network N+1 queries. SenseBridge is serverless and on-device, so
a finding in those categories usually means something worse than a bug: a
network round-trip that should not exist at all. Escalate it as an architecture
violation rather than fixing it in place.
...
- **Force-unwrapped `URL`**: `URL(string:)!` — it returns an optional because it fails; validate and bind, never force it
- **Unvalidated external input**: data from deep links, the pasteboard, or file imports used before validation. These are the realistic untrusted-input surfaces for an app with no network — treat them with the suspicion a server would give a request body
- **Unsanitized text before display**: recognised text (OCR) and filenames are attacker-influenced content; validate before it reaches a view or a spoken string
...
- **Untyped `throws` on a closed error set**: Swift 6 supports typed throws — prefer them where the failure modes are known and finite, so callers can switch exhaustively instead of guessing:

    ```swift
    func load(id: String) throws(LoadError) -> Item {
        guard let data = try? read(from: path) else { throw .fileNotFound(id) }
...
- **Booleans or optionals where a state machine belongs**: model distinct states as an enum with associated values, so impossible states stop compiling:
...
- **Unclear at the point of use**: read the call site aloud — it should read as a phrase; omit needless words
- **Named for its type, not its role**: name methods and properties for what they mean, not what they hold
- **Global constants**: prefer `static let` on the relevant type over a free-floating global
