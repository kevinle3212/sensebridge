...
> **SenseBridge adaptation** (vendored from `AgriciDaniel/claude-seo` v2.2.0,
> MIT — see `LICENSE.txt` and `CREDITS.md`): offline audit slice only. The
> "IndexNow Protocol", "DataForSEO Integration", and "Google API Integration"
> sections below are **reference prose, not wired integrations** — each pushes
> data to or pulls credentials for an external service, so enabling any of
> them requires explicit owner consent and a `docs/PRIVACY.md` review first.
> The upstream `scripts/` (Playwright live-page renderers) were deliberately
> not vendored; commands referencing them will not run here.
