...
| Language-stats check | `.gitattributes`, `tools/check-linguist-vendored.mjs`; `npm run check:linguist` | Excludes every vendor-managed `**/skills/impeccable/**` copy from GitHub's language breakdown while asserting first-party JavaScript and TypeScript remain counted |
