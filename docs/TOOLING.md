...
| Editor config | `.vscode/extensions.json`, `.vscode/settings.json` | Recommended extensions + strict per-language formatting; excludes generated dirs (`.codegraph/`, `.gitnexus/`, `tmp/`, `logs/`) from search |
...
| CodeGraph | installed (`npm install -g @colbymchenry/codegraph`) | Free-text architecture questions (`codegraph explore`), symbol source plus caller/callee trail (`codegraph node`). Index at `.codegraph/`, gitignored; refreshed incrementally by the versioned `post-commit`/`post-checkout` hooks. Replaced Graphify, retired 2026-08-18 — see `docs/archive/TOOLING-DECISIONS.md` |
...
disabled. `gitnexus` and `glyph` still work as **CLIs** from Bash.
