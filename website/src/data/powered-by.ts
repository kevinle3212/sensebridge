 ...
 * `currentColor`. Monochrome is not merely a palette preference: Vercel is
 * monochrome by design and publishes no colour vector at all, and a single
 * `currentColor` treatment is the only one that renders every brand correctly
 * in both themes.
 *
 * GSAP, Three.js, and Lenis are real runtime dependencies (see
 * `src/scripts/motion.ts`) but are not credited here: checked by rendering
 * them at 14px, not by assumption. GSAP's dedicated mark just re-draws the
 * word "GSAP" — redundant beside the text label it would sit next to.
 * Three.js's faceted-diamond mark reads as a smudge that small. Lenis
 * publishes no compact glyph at all, only a 1360x336 wordmark. Re-check
 * before adding one back — a 22px+ shields.io badge is a different scale and
 * a different judgment call; that's what README.md's stack badges use for
 * GSAP and Three.js instead.
 *
