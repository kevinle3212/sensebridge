...
function loadGraph(graphPath) {
  ...
  const links = (raw.links ?? []).filter(
    (link) =>
      kept.has(link.source) && kept.has(link.target) && link.source !== link.target,
  );
...
function renderSvg({ nodes, edges, positions, commit, animated = true }) {
  ...
  const structure = buckets
    .map((paths) => `<path class="w" d="${paths.join("")}"/>`)
    .join("");
  const flow = animated
    ? buckets
        .map(
...
function main() {
  ...
  const graphPath = resolve(
    REPO_ROOT,
    readFlag("--graph", "graphify-out/graph.json"),
  );
...
function verify(outPath, staticPath) {
  ...
  for (const [path, animated] of [
    ...
    for (const problem of [
      ...structureProblems(svg),
      ...renderProblems(svg, { animated }),
    ]) {
